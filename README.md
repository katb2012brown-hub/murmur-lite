...
- **Persistent memory** — 9 editable memory slots + 2 skill files (partner identity, user profile), auto-extracted as context fills. Add to or rewrite any slot directly in the UI. (A tenth slot quietly holds the auto-generated landing greeting — saved, but not shown, since it's app cosmetic rather than memory you manage.)
- **End session** — header button that runs memory extraction on the current thread. Use it when you're leaving a thread and want this session folded into memory before you go, instead of waiting for auto-extraction to trigger on its own. Extraction isn't free — each press is a Haiku pass over the thread — so it's a once-as-you-leave action, not something to hit repeatedly mid-conversation.
...
- **Voice** — free local Kokoro TTS by default (21 voices, runs offline on your machine, no API key; ~86MB model downloads on first use). Optional Edge TTS (free, cloud, 22 voices across British, American, Australian, Irish, Indian and Canadian accents) and optional Cartesia Sonic 3 live voice. Groq Whisper for speech-to-text in the desktop app (free tier; Web Speech API also works in a Chrome browser).
...
*Nine structured memory slots you can view and edit, plus two skill files (companion identity, partner profile). Auto-extracted from conversation as context fills. (A tenth slot holds the companion-written landing greeting — saved server-side but not shown here, since it's app cosmetic rather than memory you manage.)*
